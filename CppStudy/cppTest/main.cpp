...
namespace STL_SET_MAP
{
	...
	void test3()
	{
		std::multimap<double, int> map1;
		std::unordered_multimap<int, double> map2;

		map1.insert({ 1.1, 1 });
		map1.insert({ 1.1, -1 });
		map1.insert({ 1.1, -1 });				// 可以插入重复元素；
		map1.insert({ 1.1, 10 });
		map1.insert({ 1.1, 11 });

		auto iter = map1.find(1.1);

		// count()方法——返回multiset中某一个键对应的值的个数：
		std::cout << "map1.count(1.1) == " << map1.count(1.1) << std::endl;
		for (unsigned i = 0; i < map1.count(1.1); ++i)			// 遍历某个键对应的所有值：
...
{   
	...
	debugDisp("please input a std::string:");
	std::cin >> str2;
	debugDisp("str1 == ", str1);
	debugDisp("str2 == ", str2);
