...
void start_syntax_keyword(void) 
{
	...
	while (3 == interfaceLevel)
	{
		...
		cout << "0: test0: 关键字const" << endl;
		cout << "1: test1: 关键字static" <<endl;
...
void test0(void) 
{
	...
	cout << "\n\n\n\n" << endl;
	cout << "test0: 关键字const" << endl;


	cout << "\t顶层/底层const" << endl;
	// 顶层const：指针本身是一个常量 
	cout << "\t\tint* const cpi1 = &numi1;	*cpci1 == " << *cpi1 << "——顶层const"<< endl;
	...
	cout << "\t\tconst int* pci1 = &numi1; *pci1 == " << *pci1<< "——底层const"<< endl;
	pci1 = &numi2;												// 底层const指针可以指向其他位置：
	cout << "\t\tpci1 = &numi2; *pci1 ==  " << *pci1 << endl;


	// 顶层、底层const可以同时出现——指针本身是常量，指向的数据也是常量。
	cout << "\t\tconst int* const cpci1 = &numi1; *cpci1 == " << *cpci1<< "——顶层、底层const可以同时存在" << endl;
	cout << endl;
